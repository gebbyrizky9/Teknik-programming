public class Restaurant { // sesudah diperbaiki
            ...
            if (nama_makanan[i].equalsIgnoreCase(nama)) {
                if (stok[i] >= jumlah) {
                    ...
                    System.out.println("Pemesanan " + jumlah + " " + nama + " Berhasil!\nTotal harga: "
                            + penjumlahanpemesanan(harga, jumlah));
                } else {
    ...
    public double penjumlahanpemesanan(double harga, int jumlah) { // saya mencoba menambahkan fitur baru yaitu total
                                                                   // harga, bisa dihiraukan saja (hanya mencoba-coba)
        return (harga * jumlah);
    }
